<x-job-layout title="Cari Lowongan Pekerjaan">
    <div class="mx-auto max-w-5xl px-4 py-8 sm:px-6 lg:px-8">
            ...
            <div class="mt-4 flex flex-col gap-4 md:flex-row md:items-end rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
                ...
                <div class="flex-1">
                    ...
                    <select name="category" class="w-full rounded-xl border border-slate-200 bg-slate-50 px-4 py-2.5 text-sm text-slate-700 outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 appearance-none">
                        ...
                        <!-- Kategori dinamis bisa ditambahkan di sini nantinya -->
                    </select>
                </div>
        ...
        <div class="space-y-4">
            @forelse($jobs as $job)
                <div class="group flex flex-col sm:flex-row gap-5 rounded-2xl border border-slate-200 bg-white p-6 shadow-sm transition-all hover:shadow-md">
                    
                    <!-- Company Logo Placeholder -->
                    <div class="flex h-16 w-16 flex-shrink-0 items-center justify-center rounded-xl bg-slate-900 text-white shadow-sm overflow-hidden border border-slate-800">
                    ...
                    <div class="flex-1">
                        <!-- Title & Company -->
                        <div>
                            <h2 class="text-lg font-bold text-slate-900 group-hover:text-indigo-900 transition-colors">
                                <a href="{{ route('jobs.show', $job->id) }}">{{ $job->title }}</a>
                            </h2>
                            <div class="mt-2 inline-block rounded-md bg-emerald-50 px-2.5 py-1 text-xs font-bold text-emerald-700">
                        ...
                        <div class="mt-5 flex flex-wrap gap-2">
                            <!-- Jenis Pekerjaan Badge -->
                            <span class="rounded-full bg-blue-50 px-3.5 py-1.5 text-xs font-semibold text-blue-700">
                                {{ $job->type }}
                            </span>
                            
                            <!-- Tambahan Badge jika ada requirements (contoh saja, bisa diubah sesuai kebutuhan) -->
